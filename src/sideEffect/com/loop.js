...
const initPlayingRoom = (players): Room_Playing => {
  players = players.slice().sort((a, b) => (a.user.name < b.user.name ? 1 : -1))
    ...
    case 'waiting':
      // start the game if possible
      {
        ...
        const playersReady = Object.keys(room.pending_players)
          ...
          .filter(
            player => player.tic > Date.now() - PARAM.WAITING_PING_DELAY * 2
          )
    ...
    case 'playing':
      ...
      if (action) {
        ...
        if (newRoom.game.state === 'over') {
          return await setRoom({
            ...initWaitingRoom(),
            archive: [archiveGame(newRoom), ...room.archive.slice(0, 3)],
          })
        } else {
          return await setRoom({
            ...newRoom,
