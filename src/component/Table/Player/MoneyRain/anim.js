...
export class MoneyRain extends React.Component {
    ...
    particules = particules
      .map(particule => {
        ...
        if (particule.pos.z > 3) {
          // position
          particule.pos.x =
            pos0.x + Math.sin(t * aRho + rho) * Math.sin(t * aPhy + phy) * r

          particule.pos.y =
            pos0.y + Math.cos(t * aRho + rho) * Math.sin(t * aPhy + phy) * r

          particule.pos.z =
            pos0.z -
            Y_VELOCITY * t +
            -Math.abs(Math.cos(t * aPhy + phy) * Math.cos(t * aPhy + phy)) * 20
          ...
          particule.ry =
            ...
            Math.max(
              ...
              Math.min(
                1,
                1 - (particule.t - DURATION * 0.75) / (DURATION * 0.08)
              )
            )
        }
        ...
        if (particule.t > DURATION * 0.95)
          particule.opacity = (DURATION - particule.t) / (DURATION * 0.05)
  ...
  render() {
    return (
      <SimpleMoneyRain particules={this.state.particules} {...this.props} />
    )
  }
}
